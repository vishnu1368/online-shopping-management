    ...
    function try_login(mail_id,password, callback){
        ...
        conn.query("select password from customer where mail_id = '" + mail_id +"'", function(err, res){
            if(err){
    ...
    function try_change_password(mail, password, callback){
        var conn = require("./get_dbconnection.js")
        conn.query("")
        conn.query("update customer set password = '"+password +"' where mail_id = '" + mail + "'", function(err, res){
            if(err){
